...
labels: ["ros2", "humanoid", "tasks", "planning"]
links:
...
files:
  ...
  - history/prompts/001-ros2-humanoid-system/2-ros2-nervous-system-tasks.tasks.prompt.md
tests:
  - null
---
...
/sp.task Module 1 – The Robotic Nervous System (ROS 2)
Weeks 3–5 · Detailed Execution Tasks

Create: numbered task list with subtasks, exact dependencies, estimated time (in hours), deliverables, and clear milestones.

Tasks must be ordered so that the final result is a fully working "Hello Humanoid" demo (RViz + 500 Hz fake joints + gamepad teleop).

Decisions needing documentation:
- ROS 2 distro choice (Humble vs Jazzy) + rationale
- Python-only vs mixed C++/Python packages
- Use of composition vs lifecycle nodes for the teleop node

Testing strategy per task:
- Unit tests (pytest) for every node
- Integration test: colcon test + GitHub Actions badge
- Final acceptance: ros2 topic hz /joint_states > 480 Hz, latency < 10 ms with gamepad

Technical details:
- Research-concurrent (use web_search / browse_page while doing tasks)
- All code 100 % ROS 2 Humble on Ubuntu 22.04
- Output format: Docusaurus-ready Markdown with YAML frontmatter + collapsible code blocks
- Organize tasks in 4 phases: Research → Foundation → Analysis → Synthesis
...
- ✅ Impact: Created detailed execution tasks for Module 1 with 4 phases, dependencies, time estimates, deliverables, and milestones
- 🧪 Tests: null
- 📁 Files: Created comprehensive tasks.md with 20+ detailed tasks organized in 4 phases
- 🔁 Next prompts: Begin implementation of Task 1.1 - ROS 2 Distribution Research
- 🧠 Reflection: The tasks are well-structured with clear dependencies and measurable acceptance criteria for the final "Hello Humanoid" demo
...
- Next experiment (smallest change to try): Start implementation of the research phase tasks